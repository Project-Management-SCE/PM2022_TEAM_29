...
from forms import signupForm, SignOutForm, LoginForm, signupFormOrg, DeleteVolunteerForm, DeleteOrganizationForm, \
    DeleteFieldForm, UpdateDonationForm, DivideDonationForm, UpdateRatingForm, AddFieldForm, SearchForVolunteerForm, \
    ReportHoursForm
...
def insert_volunteer(username, password, age, location, phone, name, hobby):
    try:
        cursor.execute(
            "INSERT INTO `volunteer` (username, password, age, location, phone, name, hobby) VALUES(?, ?, ?,?,?,?,?)",
...
def registerOrg():
    ...
    if request.method == 'POST':
        ...
        if cursor.fetchone() is not None:
            ...
            insert_organization(str(username), str(password), str(name), str(age), str(location), str(phone),
...
def insert_organization(username, password, age, location, phone, name, maxvol, hobby):
    try:
        cursor.execute(
            "INSERT INTO `organization` (username, password, age, location, phone, name, maxvol, hobby) VALUES(?, ?, ?, ?,?,?,?,?)",
...
def deleteField():
    ...
    if form.validate_on_submit():
        ...
        cursor.execute("SELECT * FROM `Fields` WHERE `name` = ?",
                       (nameField,))
        # if form.validate_on_submit():
        if cursor.fetchone() is not None:
        ...
        cursor.execute("DELETE FROM 'Fields' WHERE name=?", (nameField,))
        conn.commit()
    ...
    if request.method == 'POST':
        ...
        if post_id is not None:
            ...
            cursor.execute("UPDATE 'report' SET status=? WHERE voll=? AND orgg=? AND datte=?",
                           ("ok", post_id[0], session["userorg"], post_id[1],))
            conn.commit()
            aa = cursor.execute("SELECT * FROM `report` WHERE voll=? AND orgg=? AND datte=? ",
                                (post_id[0], session["userorg"], post_id[1],)).fetchall()
...
def updateMaxHour():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("UPDATE 'hours' SET limitt=? WHERE volname=? and orgname=?",
                       (Maxhour, username, session["userorg"],))
        conn.commit()
    flash("successfully updated!")
...
def showApplications():
    ...
    if request.method == 'POST':
        ...
        if post_id is not None:
            post_id = str(request.form.get('ok'))
            cursor.execute("UPDATE 'apply' SET hobby=? WHERE orgname=? AND volname=?",
                           ("ok", session["userorg"], post_id,))
            conn.commit()
...
def showStatus():
    ...
    if request.method == 'POST':
        ...
        print(text)
        print(post_id)
        cursor.execute("UPDATE 'report' SET hour=? WHERE orgg=? AND voll=? AND datte=?",
                       (text, post_id[0], session["uservol"], post_id[1],))
        conn.commit()
...
def applyToOrg():
    ...
    if request.method == 'POST':
        ...
        if cursor.fetchone() is not None:
            ...
            apply(session["wanted"], str(name), str(age), str(location), str(email), str(phone), str(meen), str(hobby),
                  str(id), str(job))
            cursor.close()
            conn.close()
    return render_template('applyToOrg.html', form=form)
...
def apply(orgname, volname, age, location, phone, email, meen, hobby, id, job):
    try:
        cursor.execute(
            "INSERT INTO `apply` (orgname, volname, age, location, phone, email,meen, hobby, id, job) VALUES(?, ?, ?,?,?,?,?,?,?,?)",
